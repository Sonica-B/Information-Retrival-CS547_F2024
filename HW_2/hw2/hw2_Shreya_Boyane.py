...
COLLABORATORS = [
    ('Ankit Gole', 'Helped me correct syntax')
    ]
...
class BetterIndex(object):
    ...
    def wildcard_search_or(self, text):
        ...
        # For each token, search the binary tree for matching files
        for token in tokens:
            rotated_token = self._rotate(token)
            # Crawl the tree to find matches
            matching_files = crawl_tree(self._bt.root, rotated_token)
            result.update(matching_files)

        return [self._documents[doc] for doc in result]

        # tokens = self.tokenize(text)
        #
        # for token in tokens:
        #      rotate = self._rotate(token)
        #      permutation = self._permute(rotate)
        #      matches =  crawl_tree(, rotate)
        #      if matches: return self._documents.update(matches)
    ...
    def wildcard_search_and(self, text):
        ...
        for token in tokens:
            ...
            if result is None:
                result = set(matching_files)
            else:
                result.intersection_update(matching_files)
        ...
        # for token in tokens:
        #      matches =  crawl_tree(self._bt.root,self._rotate(token))
        #      if matches:
        #          if result is None: result = set(matches)
        #          else: return result.intersection_update(matches)
        #
        return [self._documents[doc] for doc in result]
